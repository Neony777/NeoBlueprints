...
public final class NetworkHandler {
    ...
    private static void handleSyncBlueprints(SyncBlueprintsPayload payload, IPayloadContext ctx) {
        ctx.enqueueWork(() -> BlueprintConfig.replaceClientCache(payload.definitions()));
    }
    ...
    public static void sendBlueprintsTo(ServerPlayer player) {
        try {
            PacketDistributor.sendToPlayer(player, new SyncBlueprintsPayload(BlueprintConfig.all()));
        } catch (Exception e) {
